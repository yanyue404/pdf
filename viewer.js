...
var PDFViewerApplication = {
  ...
  setTitleUsingUrl: function setTitleUsingUrl() {
    ...
    var title = (0, _ui_utils.getPDFFileNameFromURL)(url, '');//取的文件名
    ...
    // this.setTitle(title);//此处会把文件名设置为title,必须注释掉
  },
  ...
  load: function load(pdfDocument) {
    ...
    pdfDocument.getMetadata().then(function (_ref5) {
      _ref5.info.Title='';//注释：pdf文件自带的title信息，此处的title会拼接在viewer.html的<title></title>标签的内容前面，将其置空
      var info = _ref5.info,
  ...
  validateFileURL = function validateFileURL(file) {
    ...
    try {
      ...
      if (origin !== viewerOrigin && protocol !== 'blob:') {
        ...
        // file = file.replace(origin, viewerOrigin);
      }
      return file;
    } catch (ex) {
...
function webViewerInitialized() {
  ...
  var title;//注释：添加内容
  title = 'title' in params ? params.title : '';//注释：添加内容
  PDFViewerApplication.setTitle(title);//注释：添加内容
...
function () {
  ...
  _createClass(PDFPageView, [{
    ...
    value: function draw() {
      ...
      console.log(this.renderer, _ui_utils.RendererType.SVG );
